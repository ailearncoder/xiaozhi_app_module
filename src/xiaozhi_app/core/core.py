...
class Thing:
    def __init__(self, 
        ...
        if os.path.exists('manifest.json'):
            with open('manifest.json', 'r') as f:
                ...
                for item in self._config.get_items():
                    if hasattr(self, f"config_{item[0]}") and callable(getattr(self, f"config_{item[0]}")):
                        getattr(self, f"config_{item[0]}")(item[1])
        self.title = title
    ...
    def _handle_save_config(self, json_data:Dict, uuid):
        ...
        if plugin_name == self.name:
            if self._config:
                core_log.debug(f"Saving config: {self._config.to_json()}")
    ...
    def get_config_default(self, key:str, default:Any = None):
        try:
            if self._config:
                return self._config.get_item_value(key)
        except KeyError:
            pass
        return default
